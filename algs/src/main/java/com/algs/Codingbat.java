...
public class Codingbat {
    ...
    */
    ...
    /* We have a number of bunnies and each bunny has two big floppy ears.
       ...
       bunnyEars(2) → 4
    */
    public int numberOfEars(int numberOfBunnies) {
    ...
    */
    ...
    /* We have bunnies standing in a line, numbered 1, 2, ... The odd bunnies (1, 3, ..)
       ...
       bunnyEars2(2) → 5
     */
    public int numberOfEarsOfMutatedBunnies(int nBunnies) {
        if (nBunnies == 0) {
            return 0;
        }
        if (nBunnies % 2 == 1) {
    ...
    /* We have triangle made of blocks. The topmost row has 1 block, the next row
       ...
       triangle(2) → 3
     */
    int getBlocksNumber(int rowNumber) {
        if (rowNumber == 0) {
            return 0;
        }
    ...
    int count8(int n) {
        ...
        int lastDigit = n % 10;
        int newN = n / 10;
        if ((newN % 10) == 8 && lastDigit == 8) {
            return 2 + count8(newN);
        }
        if (lastDigit == 8) {
            return 1 + count8(newN);
        } else {
            return count8(newN);
        }
    }
